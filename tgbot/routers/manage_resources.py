...
from keyboards.manage_resources_resources_keyboard import manage_resources_resources_keyboard, EditResourceCallbackFactory
...
from config.var_config import CREATE_RESOURCE_CATEGORIES_ON_PAGE, EDIT_RESOURCE_RESOURCES_ON_PAGE
...
async def edit_resource_callback_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        reply_markup=manage_resources_resources_keyboard(resources=resources[0:5], user_lang=callback.from_user.language_code, total_pages=total_pages, page=1)
    )
    await state.set_state("resource_id")
...
async def edit_resource_page(callback: CallbackQuery, state: FSMContext, callback_data: EditResourceCallbackFactory):
    ...
    await callback.message.answer(
        ...
        reply_markup=manage_resources_resources_keyboard(
            resources=resources, 
            user_lang=callback.from_user.language_code, 
            total_pages=total_pages, 
            page=int(current_page))
