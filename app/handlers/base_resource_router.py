...
from database.managers import CategoryManager, FavoriteManager, ResourceManager, ResourceRatingManager
...
class BaseResourceRouter(ABC):
    ...
    async def _handle_add_favorite(self, user_id: str, resource_id):
        await FavoriteManager.create(
            FavoriteSchema(user_id=user_id, resource_id=resource_id),
        )
    ...
    async def _handle_rate(self, user_id: str, resource_id, rating: int):
        existing = await ResourceRatingManager.get_one(
            user_id=user_id,
    ...
    async def _show_category_list(
        ...
        await state.update_data(
            total_categories_pages=total_pages,
    ...
    async def _show_resource_list(
        ...
        await state.update_data(
            category_id=category_id,
            resources=resources,
            total_resources_pages=total_pages,
        )
